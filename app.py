...
def create_app():
        ...
        if request.method == "POST":
            ...
            if user:
                try:
                    ...
                    ok = user.password == password  # old plain pw
    ...
    def residents_list():
        ...
        rows = query.order_by(Resident.last_name.asc()).all()
    ...
    def inventory_new():
        ...
        # units for the select in your form
    ...
    def inventory_export():
        ...
        for it in rows:
            ...
            thr = it.low_stock_threshold or ""
            if show == "low":
                is_low = (qty <= (it.low_stock_threshold or 0)) if it.low_stock_threshold is not None else False
                if not is_low:
                    continue
            w.writerow([it.name, qty, it.unit or "", thr])
        return Response(
            out.getvalue(),
            mimetype="text/csv",
    ...
    def menu_scheduler():
        ...
        inv = InventoryItem.query.order_by(InventoryItem.name.asc()).all()
    ...
    def planned_menu_week():
        ...
        prev_url = url_for("planned_menu_week", base=(week_start - timedelta(days=7)).isoformat())
        next_url = url_for("planned_menu_week", base=(week_start + timedelta(days=7)).isoformat())
